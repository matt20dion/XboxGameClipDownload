...
$gameVidSaveLocation="C:\XBox One Games"
...
foreach($c in $content){
	...
	$team=$line.team
    $vidFileName = "$gameShort`_$recordedwhen`_$profileID`_$team.mp4"
    # $stFileName = "$gameShort`_smallThumbnail_$recordedwhen.png"
    $ltFileName = "$gameShort`_$recordedwhen`_$profileID`_$team.png"
    ...
    if(Test-Path $vidSaveLocation) {
        #do nothing, the fileset was already downloaded
       
    }
	elseif($recordedWhenEST -lt (get-date).AddHours(-107) -or $recordedWhenEST -gt (get-date).AddHours(-58))
	{
	#do nothing, the fileset was already downloaded
	}
	elseif($gameShort -eq "Xbox One Game")
	{
	#do nothing, not a madden clip
	}
    else{
